...
// ✅ GET /produtos retorna { produtos: [...] }, não array direto
interface RespostaProdutos { produtos: Produto[]; }
...
export default function Estoque() {
  ...
  const fetchProdutos = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get<RespostaProdutos>('/produtos');
  ...
  // ─── POST /produtos  →  multipart/form-data (multer obriga arquivo real) ──────
  async function handleCadastrar() {
    ...
    try {
      ...
      formData.append('nomeProduto',    nomeProduto.trim());
      formData.append('idCategoria',    idCategoria);
      formData.append('idFornecedor',   idFornecedor);
      formData.append('valor',          String(parseFloat(valor.replace(',', '.'))));
      formData.append('dataVencimento', dataVencimento);
      formData.append('quantidade',     quantidade);
      formData.append('qtdMin',         qtdMin);
      formData.append('qtdMax',         qtdMax);

      // ⚠️ Backend exige req.file via multer. Substitua pelo react-native-image-picker.
      if (form.imagemProduto) {
        formData.append('imagemProduto', {
          uri:  form.imagemProduto,
          type: 'image/jpeg',
          name: 'produto.jpg',
        } as any);
      }
      ...
      Alert.alert('Erro', 'Não foi possível cadastrar. Verifique os campos e tente novamente.');
    } finally {
      setSaving(false);
    }
  }
          ...
          <View style={styles.overlay}>
            <View style={styles.sheet}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <TextInput style={styles.input} placeholder="Ex: Paracetamol 500mg" placeholderTextColor="#aaa"
                  value={form.nomeProduto} onChangeText={v => setForm(p => ({ ...p, nomeProduto: v }))} />

                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.label}>ID Categoria *</Text>
                    <TextInput style={styles.input} placeholder="Ex: 1" placeholderTextColor="#aaa" keyboardType="numeric"
                      value={form.idCategoria} onChangeText={v => setForm(p => ({ ...p, idCategoria: v }))} />
                  </View>
                  <View style={{ width: 10 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.label}>ID Fornecedor *</Text>
                    <TextInput style={styles.input} placeholder="Ex: 1" placeholderTextColor="#aaa" keyboardType="numeric"
                      value={form.idFornecedor} onChangeText={v => setForm(p => ({ ...p, idFornecedor: v }))} />
                  </View>
                </View>
                ...
                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput style={styles.input} placeholder="0,00" placeholderTextColor="#aaa" keyboardType="decimal-pad"
                      value={form.valor} onChangeText={v => setForm(p => ({ ...p, valor: v }))} />
                  </View>
                  <View style={{ width: 10 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.label}>Vencimento *</Text>
                    <TextInput style={styles.input} placeholder="AAAA-MM-DD" placeholderTextColor="#aaa"
                      value={form.dataVencimento} onChangeText={v => setForm(p => ({ ...p, dataVencimento: v }))} />
                  </View>
                </View>
                ...
                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput style={styles.input} placeholder="0" placeholderTextColor="#aaa" keyboardType="numeric"
                      value={form.quantidade} onChangeText={v => setForm(p => ({ ...p, quantidade: v }))} />
                  </View>
                  <View style={{ width: 10 }} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.label}>Qtd. mínima *</Text>
                    <TextInput style={styles.input} placeholder="0" placeholderTextColor="#aaa" keyboardType="numeric"
                      value={form.qtdMin} onChangeText={v => setForm(p => ({ ...p, qtdMin: v }))} />
                  </View>
                </View>
                ...
                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
                    ...
                    <TextInput style={styles.input} placeholder="0" placeholderTextColor="#aaa" keyboardType="numeric"
                      value={form.qtdMax} onChangeText={v => setForm(p => ({ ...p, qtdMax: v }))} />
                  </View>
                  <View style={{ width: 10 }} />
                  {/* ⚠️ Integre react-native-image-picker para upload real */}
                  <View style={{ flex: 1 }}>
                    <Text style={styles.label}>Imagem (URL)</Text>
                    <TextInput style={styles.input} placeholder="https://..." placeholderTextColor="#aaa"
                      value={form.imagemProduto} onChangeText={v => setForm(p => ({ ...p, imagemProduto: v }))} />
                  </View>
                </View>
