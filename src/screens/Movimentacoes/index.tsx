...
interface Movimentacao {
  ...
  idFornecedor?: number | null;   // ✅ campo real do banco (opcional)
}
...
interface FormMov {
  ...
  dataValidade: string;
  descricao:    string;
  idFornecedor: string;           // ✅ campo extra aceito pelo backend
}

const FORM_VAZIO: FormMov = {
  idProduto: '', tipo: 'ENTRADA', quantidade: '',
  dataValidade: '', descricao: '', idFornecedor: '',
};

// Resposta tipada do GET /movimentacoes
interface RespostaListar {
...
export default function Movimentacoes() {
  ...
  // ─── GET /movimentacoes  →  { movimentacoes: [...] } ─────────────────────────
  const fetchMovs = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get<RespostaListar>('/movimentacoes');
  ...
  // ─── POST /movimentacoes ─────────────────────────────────────────────────────
  // Body aceito: { idProduto, tipo, quantidade, dataValidade?, descricao?, idFornecedor? }
  async function handleSalvar() {
    const { idProduto, tipo, quantidade, dataValidade, descricao, idFornecedor } = form;
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardRow}>
          <Text style={styles.cardInfo}>
            Produto ID: <Text style={styles.cardVal}>{item.idProduto}</Text>
          </Text>
          <Text style={styles.cardInfo}>
            Qtd: <Text style={styles.cardVal}>{item.quantidade}</Text>
          </Text>
        </View>
        {item.descricao   && <Text style={styles.cardDesc}>{item.descricao}</Text>}
        {item.idFornecedor && (
          <Text style={styles.cardDesc}>Fornecedor ID: {item.idFornecedor}</Text>
        )}
                ...
                <View style={styles.tipoRow}>
                  {(['ENTRADA', 'SAIDA'] as const).map(t => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.tipoOpt,
                        form.tipo === t && (t === 'ENTRADA' ? styles.tipoOptEntrada : styles.tipoOptSaida),
                      ]}
                      onPress={() => setForm(p => ({ ...p, tipo: t }))}
                    >
                      <Text style={[styles.tipoOptText, form.tipo === t && { color: COLORS.white }]}>
                        {t}
                      </Text>
                    </TouchableOpacity>
                  ))}
